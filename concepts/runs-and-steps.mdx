...
<Steps>
  <Step title="running event sent">
    Before your function is called, Failpath sends a `running` event for this
    step to the dashboard.
  </Step>
  ...
  <Step title="success or error event sent">
    If the function resolves, Failpath sends a `success` event. If it throws,
    Failpath sends an `error` event and rethrows the original error so your
