...
The first argument — `"checkout"` — must match the `flow.slug` from your `.failpath/flows.json`. The `runId` ties all of the step events for this execution together on the dashboard. Use the natural identifier already available in your application: a request ID, a job ID, or a webhook delivery ID all work well.
...
Pass the flow's `flow.slug` as `flowKey`, the same `runId` you use for the rest of the run, the step's `sdkStepKey` as `stepKey`, and one of `"running"`, `"success"`, `"error"`, or `"skipped"` as `status`. This lets you fill in parts of a flow that the SDK cannot instrument automatically so the dashboard graph shows the full picture.
